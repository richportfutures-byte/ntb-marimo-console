...
def _epoch_token_value(token_data: dict[str, Any], *keys: str) -> float | None:
    for key in keys:
        value = token_data.get(key)
        if isinstance(value, (int, float)):
            return float(value)
...
def token_expires_at_epoch(token_data: dict[str, Any]) -> float | None:
    explicit_expiry = _epoch_token_value(
        token_data,
        "expires_at",
        "expires_at_epoch",
    ...
    issued_at = _epoch_token_value(
        token_data,
        "created_at",
        "created_at_epoch",
        "issued_at",
        "obtained_at",
        "_ntb_obtained_at_epoch",
    )
...
def token_expires_in_seconds(
    ...
    *,
    ...
    expires_at = token_expires_at_epoch(token_data)
...
def _with_file_timestamp_expiry_fallback(token_path: Path, token_data: dict[str, Any]) -> dict[str, Any]:
    if token_expires_at_epoch(token_data) is not None:
        return token_data
    expires_in = _numeric_token_value(token_data, "expires_in")
    ...
    enriched["_ntb_obtained_at_epoch"] = obtained_at
    enriched["_ntb_expires_at_epoch"] = obtained_at + expires_in
    return enriched


def _has_freshness_metadata(token_data: dict[str, Any]) -> bool:
    return token_expires_at_epoch(token_data) is not None
...
def validate_token_contract(
    ...
    *,
    ...
    token_data = _with_file_timestamp_expiry_fallback(token_path, token_data)
...
def _failure_reason(prefix: str, exc: BaseException) -> str:
    if isinstance(exc, SchwabTokenError) and exc.exception_class:
        return f"{prefix}:{exc.exception_class}"
...
@dataclass
class RefreshableAccessTokenProvider:
    """Lazy access-token provider with proactive refresh.

    The provider performs no I/O at construction. It reads the token file on
    first use, returns the cached token while it is fresh, and refreshes via the
    Schwab OAuth refresh-token flow when the token is expired or inside the
    ...
    _cached_token_data: dict[str, Any] | None = field(default=None, init=False, repr=False)
    _cached_access_token: str | None = field(default=None, init=False, repr=False)
    _last_refresh_at_epoch: float | None = field(default=None, init=False, repr=False)
    ...
    def load_access_token(self) -> str:
        ...
        if not result.succeeded:
            raise SchwabTokenError(result.reason or "token_refresh_failed")
        if self._cached_access_token is None:
            raise SchwabTokenError("access_token_missing")
        return self._cached_access_token

    def refresh_if_needed(self) -> TokenRefreshResult:
        try:
            token_data = self._load_cached_or_file()
        except Exception as exc:
            reason = _failure_reason("token_load_failed", exc)
            self._last_error = reason
            return TokenRefreshResult(succeeded=False, reason=reason)

        if not token_is_expired_or_near_expiry(
            token_data,
            now_epoch=self.clock(),
            skew_seconds=max(0.0, float(self.refresh_margin_seconds)),
        ):
            self._last_error = None
            return TokenRefreshResult(succeeded=True, refreshed=False)

        try:
            refreshed = self._refresh_token_file()
            self._cache_token_data(refreshed)
        except Exception as exc:
            reason = _failure_reason("token_refresh_failed", exc)
            self._last_error = reason
            return TokenRefreshResult(succeeded=False, reason=reason)

        self._last_refresh_at_epoch = self.clock()
        self._refresh_count += 1
        self._last_error = None
        return TokenRefreshResult(succeeded=True, refreshed=True)

    def token_status(self) -> dict[str, object]:
        expires_in = None
        token_data = self._cached_token_data
        if token_data is not None:
            expires_in = token_expires_in_seconds(token_data, now_epoch=self.clock())
        valid = (
            self._cached_access_token is not None
            and self._last_error is None
            and (expires_in is None or expires_in > 0)
        )
        return {
            "valid": valid,
            "expires_in_seconds": expires_in,
            "last_refresh_at": _epoch_to_iso(self._last_refresh_at_epoch),
    ...
    def _load_cached_or_file(self) -> dict[str, Any]:
        if self._cached_token_data is not None:
            return self._cached_token_data
        token_data = load_token_json(self.token_path, target_root=self.target_root)
        token_data = _with_file_timestamp_expiry_fallback(self.token_path, token_data)
        self._cache_token_data(token_data)
        return token_data

    def _cache_token_data(self, token_data: dict[str, Any]) -> None:
        self._cached_access_token = access_token_from(token_data)
        refresh_token_from(token_data)
        self._cached_token_data = dict(token_data)

    def _refresh_token_file(self) -> dict[str, Any]:
...
__all__ = [
    ...
    "load_token_json",
    "load_token_with_refresh_if_needed",
    "refresh_token_file",
    "refresh_token_from",
    "require_under_state",
    "resolve_token_path",
