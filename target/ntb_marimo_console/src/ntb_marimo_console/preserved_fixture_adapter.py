...
def _mgc_fixture_analysis() -> dict[str, object]:
    ...
    return _valid_contract_analysis_no_trade(
        ...
        pivot_level=float(historical["current_session_poc"]),
        structural_notes=(
            "MGC is bounded to fixture-safe NO_TRADE as Micro Gold; the live workstation read model requires "
            "explicit numeric DXY and cash 10Y yield gates before it can surface query-ready read-model state."
        ),
    )
