...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--json", action="store_true", help="Emit the sanitized session summary as JSON.")
...
def _session_row(contract: str, cockpit: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "query_reason": query_map.get("query_enabled_reason") or query_map.get("query_disabled_reason"),
...
def _selected_contract_status(contract: str, cockpit: dict[str, object]) -> dict[str, object]:
...
def _fixture_live_snapshot():
    ...
    return build_live_observable_snapshot_v2(
        cache_snapshot,
        expected_symbols=SYMBOL_BY_CONTRACT,
...
def _quote_record(contract: str) -> StreamCacheRecord:
    ...
    fields = (
        ...
        ("bid_size", 10),
        ("ask_size", 12),
        ("quote_time", (NOW if fresh else NOW - timedelta(seconds=90)).isoformat()),
        ("trade_time", (NOW if fresh else NOW - timedelta(seconds=90)).isoformat()),
        ("volume", 1000),
        ("open", 1.0),
        ("high", 1.5),
        ("low", 0.75),
    ...
    return StreamCacheRecord(
        ...
        message_type="quote",
        fields=fields,
        updated_at=(NOW if fresh else NOW - timedelta(seconds=90)).isoformat(),
...
def _completed_bar_state(contract: str) -> ContractBarState:
    builder = ChartFuturesBarBuilder(expected_symbols=SYMBOL_BY_CONTRACT)
    for minute in range(5):
        start = NOW - timedelta(minutes=5 - minute)
        builder.ingest(
            {
...
def _dependency_states() -> dict[str, dict[str, object]]:
    return {
        "ES": {
            "cumulative_delta": {"status": "available", "source": "fixture", "fresh": True},
            "breadth": {"status": "available", "source": "fixture", "fresh": True},
        },
        "NQ": {
            "relative_strength_vs_es": {"status": "available", "source": "fixture", "fresh": True},
        },
        "CL": {
            "eia_lockout": {"status": "available", "source": "fixture", "fresh": True},
            "cumulative_delta": {"status": "available", "source": "fixture", "fresh": True},
            "current_volume_vs_average": {"status": "available", "source": "fixture", "fresh": True},
        },
        "6E": {
            "dxy": {"status": "unavailable", "source": "fixture", "fresh": False},
            "session_sequence": {"status": "available", "source": "fixture", "fresh": True},
        },
        "MGC": {
            "dxy": {"status": "available", "source": "fixture", "fresh": True},
            "cash_10y_yield": {"status": "available", "source": "fixture", "fresh": True},
            "fear_catalyst_state": {"status": "available", "source": "fixture", "fresh": True},
        },
    }
...
def _trigger_state(contract: str) -> TriggerStateResult:
    if contract == "6E":
        ...
        blocking_reasons = ("dependency_unavailable:6E:dxy",)
    else:
...
def _premarket_brief(contract: str) -> dict[str, object]:
    return {
        ...
        "status": "READY",
        "source_context": {"required_context": [], "missing_required_context": [], "unavailable_required_context": []},
        "structural_setups": [
            {
                "id": f"{contract.lower()}_fixture_setup",
                "summary": f"{contract} fixture dry-run setup.",
                "required_live_fields": ["market.current_price", "market.completed_chart_bar"],
                "query_triggers": [
                    {
