...
- structural_notes is the trader-facing market read. It must be a single compact synthesis that simultaneously (a) states or unambiguously implies the directional_bias you are emitting, (b) names the specific numeric anchor level (a key_levels value or a numeric field from market_packet/contract_specific_extension) that defines the line in the sand for that bias, and (c) states the next concrete observation the trader is watching for that would either confirm or invalidate the read. A reader of structural_notes alone should know the bias, the anchor, and the next decision point without cross-referencing any other field.
- structural_notes must not be a generic state description. Strings of the form "Price is holding above pivot", "balanced near value", "trending above pivot with conflicting signal", "structure remains intact", or any variant that omits the numeric anchor or omits the next watch-for condition are malformed structural_notes and must not be emitted.
- structural_notes must not introduce price-action observations, levels, indicators, order-flow events, breadth values, delta values, DOM/footprint reads, news, cash-index reads, DXY reads, yield reads, or any data that is not present in market_packet or contract_specific_extension. If a claim cannot be tied to a specific structured field, do not make the claim.
- structural_notes must not override or contradict structured fields. If structural_notes implies a bias different from directional_bias, the read is malformed and must be re-emitted.
- Each entry in conflicting_signals must be a real directional tension grounded in a structured field. Each entry must (a) name the specific signal or field, (b) state the direction it pulls relative to directional_bias, (c) reference the structured field or value that creates the tension, and (d) be material to the read. Format as a single sentence; vagueness is malformed.
- Examples of malformed conflicting_signals entries that must not be emitted: "delta mixed", "balance intact", "macro neutral", "session balanced", "conflicting signal present", "structure unclear", "context shifting", or any entry that names neither a direction relative to bias nor a structured-field source. Neutral or balanced context is not a conflict and belongs in structural_notes or assumptions, not conflicting_signals. Missing data is not a conflict and belongs in Stage A disqualifiers or data_quality_flags.
- conflicting_signals exists to honestly cap evidence_score under genuine tension. Inflating the count with vague entries to artificially cap evidence_score, or omitting real conflicts to preserve a higher evidence_score, are both malformed.
...
12. rationale must positively defend the trade against a wait or a different configuration. It must include, in a single coherent paragraph drawn only from contract_analysis: (a) the specific contract_analysis field or fields that justify entry_price as preferable to waiting or to a different entry, (b) the specific contract_analysis field or structural anchor that places stop_price beyond the structural invalidation, (c) the contract_analysis.key_levels reference that justifies target_1 (and target_2 when present), (d) why setup_class fits this contract_analysis context (scalp vs intraday_swing vs session_hold) rather than an alternate class, and (e) the specific contract_analysis condition whose change would invalidate the setup.
13. Strings of the form "bullish bias supports a scalp", "long from pivot toward resistance", "structure favors the trade", or any variant that re-states direction/entry/stop/target in sentence form without naming the specific contract_analysis fields that defend each decision are malformed rationale and must not be emitted.
14. rationale must not introduce new price-action, order-flow, breadth, delta, cash-tone, DXY, yield, news, DOM, or footprint observations. The boundary in rule 11 stands. Defense in rule 12 is satisfied entirely from existing contract_analysis content. If contract_analysis is too thin to satisfy rule 12, the correct response is NO_TRADE, not prose that compensates for thin analysis.
15. rationale must not rescue a weak setup with prose. If contract_analysis.confidence_band is LOW, or if contract_analysis.evidence_score is below the threshold for the chosen confidence_band, the hard NO_TRADE rules above govern; rationale does not override them.
16. Each entry in disqualifiers must be either a deterministic lockout token (for example: stop_distance_unusually_wide, quote_stale, event_lockout_active, post_eia_settling, london_close_thin_liquidity, session_winding_down) or a monitorable token tied to a specific contract_analysis field, structural anchor, or numeric threshold. A monitorable token names what to watch and the breach condition. Examples of acceptable monitorable shapes: breadth_below_55, breadth_below_threshold, dxy_change_above_plus_30bp, dxy_inversion_against_bias, es_close_below_invalidation_anchor, relative_strength_vs_es_below_1_0, delta_inversion_against_bias, cumulative_delta_divergence_persistent, value_rejection_below_VAL, value_rejection_above_VAH, eia_settling_window_active, megacap_earnings_risk_active.
17. Strings of the form "market_conditions_change", "thesis_invalidated", "structure_failed", "weak_context", "bad_trade_location", "poor_setup", "risky_trade", "context_shifted", "confidence_low", or any narrative descriptor that does not name a monitorable field/threshold or a deterministic lockout are malformed disqualifiers and must not be emitted. If a concern cannot be expressed as a deterministic lockout or a monitorable token, encode it in rationale or treat it as a NO_TRADE condition; do not list it in disqualifiers.
18. disqualifiers entries must be distinct in meaning. Do not list the same concern under two redundant tokens.
19. disqualifiers must not be used to suggest alternate trades or future setups. They are conditions that would invalidate this proposed_setup, nothing else.
