...
from ..schemas.cl import CLContractMetadata
from ..validation import validate_historical_packet
from .es import CompiledPacketArtifact
from .models import CLContractExtensionInput, CLHistoricalDataInput, CLManualOverlayInput

CL_CANONICAL_CONTRACT_METADATA = CLContractMetadata.model_validate(
    {
        "$schema": "contract_metadata_v1",
        "contract": "CL",
        "tick_size": 0.01,
        "dollar_per_tick": 10.0,
        "point_value": 1000.0,
        "max_position_size": 2,
        "slippage_ticks": 2,
        "allowed_hours_start_et": "09:00",
        "allowed_hours_end_et": "14:15",
    }
)


def _coerce_historical_input(
    payload: CLHistoricalDataInput | Mapping[str, Any],
) -> CLHistoricalDataInput:
    if isinstance(payload, CLHistoricalDataInput):
        return payload
    return CLHistoricalDataInput.model_validate(dict(payload))


def _coerce_overlay_input(
    payload: CLManualOverlayInput | Mapping[str, Any],
) -> CLManualOverlayInput:
    if isinstance(payload, CLManualOverlayInput):
        return payload
    return CLManualOverlayInput.model_validate(dict(payload))


def _coerce_extension_input(
    payload: CLContractExtensionInput | Mapping[str, Any],
) -> CLContractExtensionInput:
    if isinstance(payload, CLContractExtensionInput):
        return payload
    return CLContractExtensionInput.model_validate(dict(payload))
...
def _derive_realized_volatility_context(historical: CLHistoricalDataInput) -> str:
    if historical.avg_20d_session_range <= 0:
        raise ValueError("avg_20d_session_range must be > 0 to derive realized_volatility_context.")
    if historical.session_range < 0:
        raise ValueError("session_range must be >= 0 to derive realized_volatility_context.")
    realized_volatility_ratio = historical.session_range / historical.avg_20d_session_range
    if realized_volatility_ratio >= 1.2:
        return "elevated"
    if realized_volatility_ratio <= 0.6:
        return "compressed"
    return "normal"


def _overlay_provenance(
    overlay: CLManualOverlayInput,
    field_name: str,
    *,
...
def _build_field_provenance(overlay: CLManualOverlayInput) -> dict[str, dict[str, str]]:
    return {
        ...
        "contract_metadata": {
            ...
            "field": "CL_CANONICAL_CONTRACT_METADATA",
        },
        "market_packet.timestamp": {"source": "historical_input", "field": "timestamp"},
        "market_packet.contract": {"source": "compiler_constant", "field": "CL"},
        ...
        "contract_specific_extension.contract": {"source": "compiler_constant", "field": "CL"},
        "contract_specific_extension.eia_timing": {
            "source": "upstream_extension",
            "field": "eia_timing",
        },
        "contract_specific_extension.oil_specific_headlines": {
            "source": "upstream_extension",
            "field": "oil_specific_headlines",
        },
        "contract_specific_extension.liquidity_sweep_summary": {
            "source": "upstream_extension",
            "field": "liquidity_sweep_summary",
        },
        "contract_specific_extension.dom_liquidity_summary": {
            "source": "upstream_extension",
            "field": "dom_liquidity_summary",
        },
        "contract_specific_extension.realized_volatility_context": {
            "source": "historical_input",
            "field": "session_range",
            "derivation": (
                "derive from session_range / avg_20d_session_range with thresholds: "
                ">=1.2 elevated, <=0.6 compressed, else normal"
            ),
        },
    }


def compile_cl_packet(
    historical_input: CLHistoricalDataInput | Mapping[str, Any],
    overlay: CLManualOverlayInput | Mapping[str, Any],
    extension_input: CLContractExtensionInput | Mapping[str, Any],
    *,
    ...
    realized_volatility_context = _derive_realized_volatility_context(historical)

    packet_payload = {
        "$schema": "historical_packet_v1",
        "challenge_state": manual_overlay.challenge_state.model_dump(by_alias=True, mode="json"),
        "contract_metadata": CL_CANONICAL_CONTRACT_METADATA.model_dump(by_alias=True, mode="json"),
        "market_packet": {
        ...
        "contract_specific_extension": {
            ...
            "contract": "CL",
            "eia_timing": extension.eia_timing.model_dump(by_alias=True, mode="json"),
            "oil_specific_headlines": extension.oil_specific_headlines,
            "liquidity_sweep_summary": extension.liquidity_sweep_summary,
            "dom_liquidity_summary": extension.dom_liquidity_summary,
            "realized_volatility_context": realized_volatility_context,
        },
    ...
    provenance = {
        ...
        "derived_features": {
            "realized_volatility_context": {
                "source": "historical_input",
                "algorithm": (
                    "session_range / avg_20d_session_range with thresholds: "
                    ">=1.2 elevated, <=0.6 compressed, else normal"
                ),
                "session_range": historical.session_range,
                "avg_20d_session_range": historical.avg_20d_session_range,
                "ratio": round(historical.session_range / historical.avg_20d_session_range, 4),
                "value": realized_volatility_context,
            }
        },
    }
